...
class SpawnPoints:
    ...
    def points_that_can_spawn(self, last_not_seen_time, seen_time):
        return filter(lambda x: x.could_have_spawned(last_not_seen_time, seen_time), self.spawnpoints)

    def all_matching_spanwpoints(self, seen_at):
        return filter(lambda x: x.spawns_at(seen_at), self.spawnpoints)
    ...
    def search_points_for_runner(self, last_not_seen_time, seen_time):
        ...
        first_window = filter(lambda x: x.could_have_spawned(expanded_start_window, seen_time), self.spawnpoints)
        if len(first_window) > 0:
            return first_window
        expanded_start_window = last_not_seen_time - timedelta(minutes=10)
        return filter(lambda x: x.could_have_spawned(expanded_start_window, seen_time), self.spawnpoints)
...
class SpawnPoints_could_have_spawned(unittest.TestCase):
    def test(self):
        ...
        unseen = datetime(2016, 12, 1, 2, 25, 0)
        seen = datetime(2016, 12, 1, 2, 27, 0)
        self.assertEquals(1, len(points.points_that_can_spawn(unseen, seen)))
        unseen2 = datetime(2016, 12, 1, 2, 2, 0)
        seen2 = datetime(2016, 12, 1, 2, 3, 30)
        self.assertEquals(1, len(points.points_that_can_spawn(unseen2, seen2)))
...
class SpawnPoints_expanded_start_window(unittest.TestCase):
    def test(self):
        ...
        seen = datetime(2016, 12, 1, 2, 27, 0)
        self.assertEquals(1, len(points.points_that_can_spawn(unseen, seen)))
        self.assertEquals(2, len(points.search_points_for_runner(unseen, seen)))
