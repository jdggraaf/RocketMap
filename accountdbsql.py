...
def db_find_allocatable_by_level(temp_ban_time, perm_ban_time, warn_time, blind_time, now, min_level=0, max_level=40):
    sql = "SELECT username, password, provider,iOS,model,device_id as id FROM account " \
...
def load_accounts(system_id, ban_cutoff_date):
    if not system_id:
    ...
    sql = "SELECT username,password,provider as auth,lures,rest_until,allocated,perm_banned,temp_banned,last_login," \
          ...
          "ORDER BY username;"
    return do_fetch_all(sql, (system_id, ban_cutoff_date, ban_cutoff_date))


def load_accounts_for_lures(system_id, ban_cutoff_date):
    if not system_id:
        raise ValueError("need system_id")
    sql = "SELECT username,password,provider as auth,lures,rest_until,allocated,perm_banned,temp_banned,last_login," \
          "blinded,behaviour,`level` " \
...
def load_accounts_for_blindcheck(system_id, ban_cutoff_date):
    if not system_id:
        raise ValueError("need system_id")
    sql = "SELECT username,password,provider as auth,lures,rest_until,allocated,perm_banned,temp_banned,last_login," \
          "blinded,behaviour,`level` " \
...
def db_delete_pokestop(pokestop):
    do_update('delete from pokestop where pokestop_id=%s', (pokestop))
...
class DbtestAllocatableWithOneBanShadowWarnWindowsAndAccountInAllocationWindow(unittest.TestCase):
    def test(self):
        ...
        now = datetime.datetime(2012, 9, 18, 21, 0, 0)
        ban_release_time = datetime.datetime(2012, 9, 16, 23, 1, 0)
        warn_release_time = datetime.datetime(2012, 9, 16, 23, 1, 0)
        blinded_release_time = datetime.datetime(2012, 9, 16, 23, 1, 0)
...
class DbtestBlindedReallocation(unittest.TestCase):  # is this meaningful
    def test(self):
        ...
        now = datetime.datetime(2012, 9, 15, 21, 1, 0)
        blinded_release_time = datetime.datetime(2012, 9, 14, 21, 1, 0)
