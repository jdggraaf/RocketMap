...
def inventory_discardable_pokemon(map_objects, buddy_id):
    inv_pokemon = inventory_pokemon(map_objects)
    nonfavs = [x for x in inv_pokemon if is_discardable(x, buddy_id)]
    return nonfavs
...
def is_discardable(pkmn, buddy_id):
    data_ = pkmn["pokemon_data"]
    return "favourite" not in data_ and \
           "is_egg" not in data_ and \
           "deployed_fort_id" not in data_ and not is_keeper(pkmn) \
            and not buddy_id == pkmn["pokemon_data"]["id"]

def is_keeper(pkmn):
    return pkmn["pokemon_data"]["pokemon_id"] == 64
...
def inventory_pokedex(response):
    return inventory_item_data(response, "pokedex_entry")

def inventory_candy(response):
    return inventory_item_data(response, "candy")

def inventory_player_stats(response):
    return inventory_item_data(response, "player_stats")
