...
from luredbsql import set_lure_db_args, lures
...
def lure_bomb_get(user, position, minutes, radius=50):
    ...
    the_thread = Thread(target=lambda: ld.lure_bomb(pos, radius))
    the_thread.start()
    return "Luring at {}, be a little patitent ".format(str(pos))
