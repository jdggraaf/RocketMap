...
from pokemonhandler import s2msg, db_set_temp_banned
...
class FeedWorker(BasicFeeder):
    ...
    def berry_gym(self, pos, first_time):
        ...
        if len(filtered_needy) > 0 or (under_attack and self.defend):
            if under_attack:
...
def safe_berry_one_position(pos, worker):
    while True:
        try:
            ...
            log.warn("Account is temp banned, replacing")
            db_set_temp_banned(worker.worker.name(), datetime.datetime.now())
            worker.worker = None
            sleep(4)
