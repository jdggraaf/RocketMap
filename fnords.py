...
from behaviours import beh_handle_level_up, \
    beh_do_process_single_gmo_gym, determine_behaviour, is_pokestop, is_pokemon, \
    is_aggressive_pokestop, is_aggressive_pokemon, beh_random_bag_cleaning, discard_random_pokemon, \
    beh_spin_nearby_pokestops_with_log_map, beh_catch_all_nearby_pokemon
...
from management_errors import NoMoreWorkers, TooFarAway
...
runner_account_manager = AccountManager(args.node_name + "_runners", args.using_account_db, args.runner_accountcsv, (),
                                        args, [], [], Queue(), {})
runner_worker_manager = WorkerManager(runner_account_manager, 25, 9)
...
def run_for_it(runner, encounter_id, runner_pos, spawn_point, seen_at):
    log.info("Running for {}".format(encounter_id))
    try:
        pkmn = run_for_spawnpoints(runner, encounter_id, runner_pos)
        if pkmn:
            pokemon_queue.put(pkmn)
            return
        pkmn = run_for_spawnpoints(runner, encounter_id, spawn_point.all_matching_spanwpoints(seen_at))
        if pkmn:
            pokemon_queue.put(pkmn)
            return

        log.warn("Runner did not find {} in {} searches".format(encounter_id, len(runner_pos)))
    finally:
        runner_worker_manager.free_worker(runner)
            ...
            if "spawn_point_id" in pkmn:
                ...
                if len(possible_spawn_points) == 1:
                    ...
                    if len(possible_spawn_points) > 1:
                        ...
                        if len(runner_pos) == 0:
                            log.info(explain)
                    if len(runner_pos) > 0 and try_acquire_run_permission(encounter_id):
                        location = runner_pos[0].location()
                        try:
                            runner = runner_worker_manager.get_worker_for_location(location, is_good_shit(pkmn), False)
                            th = Thread(target=run_for_it, args=(runner, encounter_id, runner_pos, cell_spawn_points, this_map_objects))
                            th.start()
                            running = True
                        except NoMoreWorkers:
                            log.warning("No runners available to service {} at {}".format(str(pkmn), location))
                        except TooFarAway:
                            log.warning("All available workers are too far awayto service {} at {}".format(str(pkmn), location))
...
def queue_worker():
    rares = set()
    catchbot = CatchBot(args, args.node_name + "_lvlr", "levelers.txt")
