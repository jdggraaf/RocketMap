...
(
    ...
    pokemon_uid VARCHAR(50) PRIMARY KEY NOT NULL,
    ...
    stamina_max INT(11),
    cp_multiplier FLOAT,
    additional_cp_multiplier FLOAT,
    iv_defense INT(11),
    iv_stamina INT(11),
    iv_attack INT(11),
    ...
    last_no_present datetime null,
    CONSTRAINT scanspawnpoint_ibfk_1 FOREIGN KEY (gym_id) REFERENCES gym (gym_id)
);
CREATE INDEX gym_defender ON defender (gym_id);
CREATE INDEX gym_trainers ON defender (trainer_name);
...
def log_gym_change_in_db(g, previous_gym_name, kmh, distance):
    ...
    try:
        ...
        trainer_name_ = g["trainer_name"].encode('utf-8')
        id_ = g["gym_id"].encode('utf-8')
        pokemon_id_ = g["pokemon_id"]
        uid_ = g["pokemon_uid"].encode('utf-8')
...
def all_gyms():
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT gym.gym_id, gym.latitude, gym.longitude, gymdetails.name FROM gym,gymdetails WHERE gym.gym_id=gymdetails.gym_id;"
            cursor.execute(sql)
...
def update_gym_members(gym, added, removed, gym_last_previous_scan):
    ...
    try:
        ...
        for memberId, member in added.iteritems():
...
def db_load_spawn_points_for_cell(cell_id):
    connection = __gymmapconnection()
    try:
        with connection.cursor() as cursor:
            sql = "SELECT a.*,sp.s2cell,sp.altitude FROM spawnpoint a LEFT JOIN spawnpoints2 sp ON a.id = sp.id WHERE s2cell=%s"
            cursor.execute(sql, cell_id)
            return cursor.fetchall()
    finally:
        connection.close()
...
def update_gym_scaninfo_gymdata(connection, data_):
    ...
    with connection.cursor() as cursor:
        ...
        if fetchone and last_modified == fetchone["last_modified"]:
            ...
            sql = "UPDATE gym SET last_scanned=%s,last_modified=%s,team_id=%s,guard_pokemon_id=%s,gym_points=%s WHERE gym_id=%s"
            values = (last_scanned, last_modified, team_id, guard_pokemon_id, gym_points, gym_id_)
            cursor.execute(sql, values)
...
def insert_gym_details_table(connection, gym_get_info):
    ...
    description = gym_get_info.get("description", None)
...
def pokestops():
    ...
    try:
        with connection.cursor() as cursor:
            sql = "SELECT pokestop_id,latitude,longitude FROM pokestop ORDER BY longitude"
            cursor.execute(sql)
            return cursor.fetchall()
    finally:
        connection.close()
