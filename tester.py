...
from behaviours import beh_spin_nearby_pokestops, beh_catch_pokemon
...
l5account = Account2("AysOuDaBlah", "Freedom4@ll", "ptc", args, 7200, 1800, cycle(args.hash_key), None, {}, None)
...
raid_end = datetime.fromtimestamp(pokemon_for_proto.get("raid_info", {}).get("raid_end_ms") / 1000)
raid_battle = datetime.fromtimestamp(pokemon_for_proto.get("raid_info", {}).get("raid_battle_ms") / 1000)
raid_spawn = datetime.fromtimestamp(pokemon_for_proto.get("raid_info", {}).get("raid_spawn_ms") / 1000)

req = l5account.account_info().pgoApi.create_request()
x = req.get_raid_details(raid_seed=raid_seed,
                     gym_id=gym["id"],
                         lobby_id=[],
                         player_lat_degrees=pos[0],
                         player_lng_degrees=pos[1])
...
beh_catch_pokemon(l5account, l5obj, pos, first["encounter_id"], first["spawn_point_id"])
