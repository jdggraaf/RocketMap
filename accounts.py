...
class AccountManager:
    ...
    def initialize(self, accounts_file, settings_accounts):
        ...
        if self.usingdb:
            if file_accts:
                ...
                AccountManager.insert_accounts(file_accts, self.name, allocation_period)
    ...
    def __load_db_account_objects(self):
        result = []
        all_accounts = self.loader(self.name, self.ban_cutoff_date)
    ...
    @staticmethod
    def insert_accounts(accounts, system_id, allocation_duration=None, force_system_id=False, skip_assigned=False):
        now = datetime.now()
